...
class GRPOTrainer(Trainer):
    ...
    def _gather_batch_data(
        ...
        return all_prompts, all_answers, all_tasks, all_infos, all_metadata
    ...
    def _prepare_inputs(  # type: ignore
        ...
        if self._step % generate_every == 0 or self._buffered_inputs is None:
            ...
            for batch_id in range(self._next_batch_id, target_batch_id + 1):
                ...
                all_prompts, all_answers, all_tasks, all_infos, all_metadata = (
                    self._gather_batch_data(batch_offset) # type: ignore
                )
                if not all_prompts:
                    self.logger.info(
    ...
    def evaluate(
        ...
        if self.accelerator.is_main_process and self.log_completions:
            ...
            states = eval_results.state[:self.num_completions_to_print]
            ...
            if (
                ...
                wandb.log({"eval_completions": wandb.Table(dataframe=df, log_mode="INCREMENTAL")})
        ...
        if self.accelerator.is_main_process and self.log_completions:
            ...
            if (
                ...
                if len(table["prompt"]) > 0:
                    ...
                    wandb.log({"completions": wandb.Table(dataframe=df, log_mode="INCREMENTAL")})
